...
import { pharmacyData } from '../constants/constants';

export default function PharmacyCategories() {
  return (
    <>
    <Text style={{fontSize: wp(4)}}>Pharmacy by Categories</Text>
    <View style={{flex:1, flexDirection:'row',flexWrap:'wrap'}}>
              {
                pharmacyData.map((cat)=>{
                return (
                    <View style={styles.item}>
                    <TouchableOpacity className="flex items-center space-y-2">
                        <Image source={cat.image} className="rounded-3xl" style={{width: wp(40), height: wp(35)}} />
